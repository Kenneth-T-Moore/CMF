...
class Var_yi(ExplicitSystem):
    ...
    def _declare_local(self):
        ...
        self._declare_local_argument('xi', indices=[0], copy=-1)
        self._declare_local_argument('ai', indices=[0], copy=-1)
...
class Var_ai(ImplicitSystem):
    ...
    def _declare_local(self):
        ...
        self._declare_local_argument('x0')
        self._declare_local_argument('xi', indices=[0], copy=-1)
        self._declare_local_argument('yi', indices=[0], copy=-1)
    ...
    def _apply_F(self):
        ...
        xi = self.pVec(['xi',-1])[0]
        yi = self.pVec(['yi',-1])[0]
        self.uVec()[0] = numpy.log(x0-yi/xi)
        '''
    def _apply_dFdup(self, mode, arguments):
        x0 = self.pVec('x0')[0]
        xi = self.pVec('xi',-1)[0]
        yi = self.pVec('yi',-1)[0]
        ai = self.uVec()[0]
...
class Var_bi(ImplicitSystem):
    ...
    def _apply_F(self):
        ...
        ai = self.pVec('ai',-1)[0]
        bi = self.uVec()[0]
        self.fVec()[0] = numpy.log(bi)+bi-ai-xi
        '''
        def _solve_F(self):
        xi = self.pVec('xi',-1)[0]
        ai = self.pVec('ai',-1)[0]
        bi = self.uVec()[0]
        self.uVec()[0] = ai+xi-numpy.log(bi)
        '''
    def _apply_dFdup(self, mode, arguments):
        xi = self.pVec('xi',-1)[0]
        ai = self.pVec('ai',-1)[0]
        ...
        if mode == 'rev':
            if ['xi',-1] in arguments:
                self.dfVec('xi',-1)[0] += -1 * self.dpVec()[0]
            if ['ai',-1] in arguments:
                self.dfVec('ai',-1)[0] += -1 * self.dpVec()[0]
            if ['bi',-1] in arguments:
                self.dfVec('bi',-1)[0] += (1 + 1/bi) * self.duVec()[0]
